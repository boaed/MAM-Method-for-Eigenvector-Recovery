...
class ExperimentResult:
    expt_id: int
    sparsity: int   # this parameters specifies the sparsity of the eigenvalues of A 
...
'This code construct the bit testing matrix B_N prime'
...
def append_result_to_csv(result: ExperimentResult, csv_path: Path):
...
def run_single_experiment(N, r,decay_type,times_of_nnz,ratio,support_type,K,s,run,p_list,M):
...
def run_experiment_varying_sparsity(N, r, decay_type, times_of_nnz, ratio, support_type, K, s_list, p_list, runs, csv_path):
    """Run the experiment in parallel."""
    
    M = fast_create_sparse_matrix(p_list, N)  # Create sparse matrix once
    
    # Parallel execution
    results = Parallel(n_jobs=10, verbose=10, prefer="processes")(  # Use all available CPU cores
        delayed(run_single_experiment)(N, r, decay_type, times_of_nnz, ratio, support_type, K, s, run, p_list, M)
        for s in s_list for run in range(runs)
    )

    # Flatten list of results and save to CSV in bulk
...
# def plot_experiment(csv_path: Path, fig_dir: Path, param_str: str, N, r, m, K, s_list):
#     """Plot errors vs sparsity and track min/max error values for each eigenvector."""
#     df = pd.read_csv(csv_path)
#     df.columns = [col.lower() for col in df.columns]

#     # Filter for the first 4 eigenvectors (index 0 to 3)
#     df_filtered = df[df['singular_vector_index'] < 4]

#     # Compute median, min, max
#     stats = df_filtered.groupby(['sparsity', 'singular_vector_index']).agg({
#         'rel_l2_error': ['median', 'min', 'max'],
#         'linf_error': ['median', 'min', 'max'],
#         'rel_l2_inversion_error': ['median', 'min', 'max'],
#         'linf_inversion_error': ['median', 'min', 'max']
#     }).reset_index()
#     stats.columns = ['sparsity', 'singular_vector_index',
#                      'rel_l2_median', 'rel_l2_min', 'rel_l2_max',
#                      'linf_median', 'linf_min', 'linf_max',
#                      'rel_l2_inv_median', 'rel_l2_inv_min', 'rel_l2_inv_max',
#                      'linf_inv_median', 'linf_inv_min', 'linf_inv_max']
    
#     title = f'N={N}, r={r}, m={m}, K={K}'
#     file_prefix = f"{param_str}_"

#     markers = ['o', 's', 'D', '^']
#     names = ['st eigenvector', 'nd eigenvector','rd eigenvector','th eigenvector']



#     def plot_error(metric, ylabel, filename, median_col, min_col, max_col):
#         plt.figure(figsize=(10, 6))
#         for k in range(4):
#             subset = stats[stats['singular_vector_index'] == k]
#             marker = markers[k]
#             position = names[k]
#             plt.plot(subset['sparsity'], subset[median_col],
#                      label=f'{k+1} {position} ', marker=marker, linestyle='-')
#             #plt.plot(subset['sparsity'], subset[min_col],
#             #         label=f'Eigenvector {k+1} (min)', marker=marker, linestyle='--')
#             #plt.plot(subset['sparsity'], subset[max_col],
#             #         label=f'Eigenvector {k+1} (max)', marker=marker, linestyle='--')

#         plt.xlabel('Sparsity (s)')
#         plt.ylabel(ylabel)
#         plt.title(f'{ylabel} vs Sparsity\n{title}')
#         plt.xticks(np.arange(min(s_list), max(s_list) + 10, 10))
#         plt.legend()
#         plt.grid(True)
#         plt.tight_layout()
#         plt.savefig(fig_dir / f'{file_prefix}{filename}.png')
#         plt.close()

#     # Generate the plots
#     plot_error('rel_l2_error', 'rel_L2 Error (Before Inversion)', 'rel_l2_error_before_inversion', 'rel_l2_median', 'rel_l2_min', 'rel_l2_max')
#     plot_error('linf_error', 'Linf Error (Before Inversion)', 'linf_error_before_inversion', 'linf_median', 'linf_min', 'linf_max')
#     plot_error('rel_l2_inversion_error', 'rel_L2 Error (After Inversion)', 'rel_l2_error_after_inversion', 'rel_l2_inv_median', 'rel_l2_inv_min', 'rel_l2_inv_max')
#     plot_error('linf_inversion_error', 'Linf Error (After Inversion)', 'linf_error_after_inversion', 'linf_inv_median', 'linf_inv_min', 'linf_inv_max')
#     # === Summary stats per eigenvector ===
#     summary_list = []
#     for k in range(4):
#         df_k = df_filtered[df_filtered['singular_vector_index'] == k]
#         summary_list.append({
#             'Eigenvector': k + 1,
#             'Eigenvalue' : k + 1, 
#             'rel_L2_min': df_k['rel_l2_error'].min(),
#             'rel_L2_max': df_k['rel_l2_error'].max(),
#             'Linf_min': df_k['linf_error'].min(),
#             'Linf_max': df_k['linf_error'].max(),
#             'rel_L2_inv_min': df_k['rel_l2_inversion_error'].min(),
#             'rel_L2_inv_max': df_k['rel_l2_inversion_error'].max(),
#             'Linf_inv_min': df_k['linf_inversion_error'].min(),
#             'Linf_inv_max': df_k['linf_inversion_error'].max()
#         })

#     summary_df = pd.DataFrame(summary_list)
#     print("=== Max/Min Error Summary Per Eigenvector ===")
#     print(summary_df)
def plot_experiment(csv_path: Path, fig_dir: Path, param_str: str, N, r, m, K, s_list):
    """Plot errors vs sparsity for the first 4 singular vectors."""
    df = pd.read_csv(csv_path)
    df.columns = [col.lower() for col in df.columns]
    df_filtered = df[df['eigen_vector_index'] < 4]
    mean_errors = df_filtered.groupby(['sparsity', 'eigen_vector_index']).mean().reset_index()
    ...
    def plot_metric(metric, ylabel, filename):
        ...
        for k in range(4):
            ...
            marker = markers[k]
            ranks = ranked[k]
            plt.plot(subset['sparsity'], subset[metric], label=f'{k+1} {ranks}', marker = marker,linewidth=4)
        plt.xlabel('Sparsity (s)')
        plt.ylabel(ylabel)
        plt.title(f'{ylabel} vs Sparsity (s)')
        plt.xticks(np.arange(min(s_list), max(s_list), 5))
        plt.legend()
        plt.grid(True)
        plt.tight_layout()
    ...
    #summary_df.to_csv(fig_dir / f'{file_prefix}error_summary.csv', index=False)
...
def main():
    ...
    times_of_nnz = 1  # Times of nonzeros
    ratio = 0.001  # Ratio
    support_type = 'overlapping'  # Support type
    s_list = [40,44,48,52,56,60,65,70]  # List of sparsity values
    #smallest_s = s_list[0]
    p_num, p_list = number_and_list_primes(10000,3,40)
    smallest_primo  = p_list[0]
    m = np.sum(p_list)
    K = p_num  # Number of primes
    
    runs = 20  # Number of runs per sparsity value

    # Create output directories
    fig_dir = Path('./figs')
    fig_dir.mkdir(exist_ok=True)

    param_str = f'N={N}_r={r}_decay={decay_type}_nnz={times_of_nnz}_ratio={ratio}_support={support_type}_smallest_prime={smallest_primo}_K={K}_Number_of_rows_in_M={m}_s={"|".join(map(str, s_list))}_runs={runs}'
    csv_path = fig_dir / f'results_sparsity_{param_str}.csv'
    #csv_path_values = fig_dir / f'Eigenvalues_results_sparsity_{param_str}.csv'
    # Run the experiment
    run_experiment_varying_sparsity(N, r, decay_type, times_of_nnz, ratio, support_type, K, s_list, p_list,runs, csv_path)
    ...
    # Plot the results
    plot_experiment(csv_path, fig_dir, param_str, N, r, m, K, s_list)
